...
parseMixedTemplateContentContainer = function(node) {
  ...
  if(node.children && node.children.length && node.children[0].name === 'random') {
	var randomNode = node.children[0];
	var choices = _.filter(randomNode.children, function(subNode) {
		return subNode.name === 'li';
	});
	// TODO actually choose random number
	return parseMixedTemplateContentContainer(choices[0].children[0]);
  }
